...
class PayrollAnalytic(models.Model):
	...
	# método auxiliar que corta registros de parte de horas día por día y con límite de fechas
	...
	def get_divided_hours(self, contract, date_from_payslip, date_to_payslip, hi_cutstr, hf_cutstr):
		...
		# 1.1. =====> festivos
		holidays_ids = self.env['hr.holidays'].get_publicholidays_ids(date_from_payslip_c, date_to_payslip_c, country_emp_id)
		# 2. consulta
		# 2.1. =====> fechas nómina a zona horaria utc cero
		date_from_payslip_c = date_from_payslip_c.astimezone(pytz.utc)
		date_to_payslip_c   = date_to_payslip_c.astimezone(pytz.utc)
