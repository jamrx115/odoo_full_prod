...
class CustomKnowledge(models.Model):
    ...
    approver_id = fields.Many2one('custom.approver.knowledge',string="Aprobador", track_visibility="onchange")
    department_id = fields.Many2one('hr.department', string="Departamento", track_visibility="onchange")
    country_id = fields.Char(string="País ubicación de trabajo",related="name.company_id.country_id.name",store=True, copy=True)
    ...
    exam_name_id = fields.Many2one('custom.type.certification', string="Nombre del examen")
    ...
    reason = fields.Many2one('custom.reasons', string="Motivo de rechazo o de aplazamiento")
    ...
    state = fields.Selection([
        ...
        ('proceso','En proceso'),
        ('rechazada','Rechazada'),
        ('aplazada','Aplazada'),('terminada','Terminada')], string="Estado", track_visibility="onchange",default="solicitud")
    
    #Acciones para los botones
    @api.model
    def create(self, values):
        values['state'] = 'jefe'
        line = super(CustomKnowledge, self.with_context(mail_create_nosubscribe=True)).create(values)
        if line:
            template_id = self.env.ref('custom_knowledge.knowledge_email_template_boss')
        if template_id:
    ...
    def action_vobo(self):
        ...
        # template = self.env.ref('custom_knowledge.knowledge_email_template_human_services')
    ...
    def action_pendiente_aprobacion(self):
        # # Find the e-mail template
        # template = self.env.ref('custom_knowledge.knowledge_email_template_management')
        # # Send out the e-mail template to the user
        # self.env['mail.template'].browse(template.id).send_mail(self.id)
        self.write({
            'state': 'pendiente',
    ...
    def action_aprobar(self):
        # # Find the e-mail template
        # template = self.env.ref('custom_knowledge.knowledge_email_template_approved')
        # # Send out the e-mail template to the user
        # self.env['mail.template'].browse(template.id).send_mail(self.id)
        self.write({
            'state': 'aprobada',
    ...
    def action_rechazar(self):
        if self.reason == False:
            ...
            # # Find the e-mail template
            # template = self.env.ref('custom_knowledge.knowledge_email_template_rejected')
            # # Send out the e-mail template to the user
            # self.env['mail.template'].browse(template.id).send_mail(self.id)
            self.write({
                'state': 'rechazada',
    ...
    def action_aplazar(self):
        if self.reason == False:
            ...
            # # Find the e-mail template
            # template = self.env.ref('custom_knowledge.knowledge_email_template_deferment')
            # # Send out the e-mail template to the user
            # self.env['mail.template'].browse(template.id).send_mail(self.id)
            self.write({
                'state': 'aplazada',
    ...
    def action_request_documents(self):
        # # Find the e-mail template
        # template = self.env.ref('custom_knowledge.knowledge_email_template_documents')
        # # Send out the e-mail template to the user
        # self.env['mail.template'].browse(template.id).send_mail(self.id)
        pass
...
class CustomApproverKnowledge (models.Model):
    _name="custom.approver.knowledge"
...
class CustomEvaluationKnowledge (models.Model):
    _name="custom.evaluation.knowledge"
    ...
    def create(self, values):
        ...
        res = super(CustomEvaluationKnowledge,self.with_context(mail_create_nosubscribe=True)).create(values)
        return res
...
class reasons (models.Model):
    ...
    @api.multi
    def unlink(self):
        range_obj = self.env['custom.knowledge']
