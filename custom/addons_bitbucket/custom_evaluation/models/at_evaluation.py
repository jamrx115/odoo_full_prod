...
class AtEvaluationSubtype (models.Model):
    _name = "at.subtype"
    _description = "Subtema de evaluaciones"

    type = fields.Selection([
        ('procesos','01_Procesos organizacionales'),
        ('operacion','02_Operación y servicios'),
        ('metodologias','03_Metodologías'),
        ('desarrollo','04_Ingeniería y desarrollo de software'),
        ('tecnologias','05_Tecnologías')], 
        string="Tema principal")
    name = fields.Char(string='Subtema', required=True)
...
class AtEvaluationExam(models.Model):
    ...
    type = fields.Selection([
        ...
        ('tecnologias','05_Tecnologías')], 
        string="Tema principal")
    subtype_id = fields.Many2one('at.subtype', string="Subtema")

    @api.multi
    @api.onchange('type')
    def _onchange_type_id(self):
        if self.type:
            return {'domain': {'subtype_id': [('type', '=', self.type)]}}
        else:
