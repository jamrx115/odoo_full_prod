...
class ResPartner(models.Model):
    ...
    vat_ref = fields.Char(string="NIT Con DV",compute="_compute_vat_ref",readonly=True,)
...
#        return super(ResPartner, self).name_search(name, args=args, operator=operator, limit=limit)
    ...
    def check_vat(self):
        #self.vat.strip()
        if not self.vat:
            raise ValidationError(u'Debe Suministrar un Numero De Documento para %s' % self.name) 
            #return True
        else:
            self.vat.strip()
            if not self.parent_id:
                if len(self.search([('company_id','=',self.company_id.id),('vat','=',self.vat)])) != 1:
                    raise ValidationError(u'El Numero %s Suministrado Para Este Tercero Ya Existe' % self.vat)
                    #raise ValidationError(u'Identificación [%s] suministrado para Tercero ya existe' %(self.vat, self.name))
                return True
            return True
